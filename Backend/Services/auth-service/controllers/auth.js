...
export const verifyEmail = async (req, res) => {
    ...
    if (otpFromredis === otp) {
      ...
      try {
        ...
        if (userData.roles.includes("DELIVERY")) {
          await publishEvent(TOPICS.USER_EVENTS, {
        ...
        console.log(`📡 Kafka Events published for ${email}`);
      } catch (kafkaErr) {
...
export const deliverySignup = async (req, res) => {
  const connection = await pool.getConnection();
  console.log("Delivery Signup api hit");

  try {
    const { full_name, email, password, role = "DELIVERY" } = req.body;
    ...
    return res.status(201).json({
      success: true,
      message: isExisting ? "Delivery role added successfully." : "Delivery partner registered successfully.",
      user: { id: userId, full_name, email, roles: assignedRolesArray },
    });
  } catch (error) {
    await connection.rollback();
